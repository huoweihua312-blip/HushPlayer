...
new theme is revealed from the upper-left brand mark and expands across the
window. The demo is opt-in through `HUSHPLAYER_THEME_REVEAL_DEMO=1` so normal
...
- Cut an expanding circular hole from the overlay, centered on the visible
  HushPlayer Logo, so the target theme appears to radiate from the brand mark.
- Use a 420 ms ease-out transition and remove the overlay when finished.
...
1. A manual theme toggle with the demo flag starts at the Logo and reveals the
   target theme across the whole window.
