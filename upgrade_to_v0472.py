...
BACKUP_FILE = PROJECT_ROOT / "app" / "ui" / "main_window.py.bak_v0471"
...
def main() -> None:
    ...
    if "HushPlayer/0.4.7.2" in text:
        print("当前文件看起来已经升级到 v0.4.7.2 了，不需要重复升级。")
        return

    if "def set_right_panel_mode" not in text:
        raise RuntimeError("没有找到 v0.4.7.1 的右侧面板切换代码。请先确认已经升级到 v0.4.7.1。")
...
'''
    ...
    print("升级完成：v0.4.7.2 歌词页打开时只隐藏右侧小歌词，不再显示右侧信息方块。")
