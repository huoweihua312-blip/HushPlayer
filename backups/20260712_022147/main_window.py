...
from PySide6.QtGui import QColor, QFontMetrics, QIcon, QKeySequence, QPainter, QPainterPath, QPen, QPixmap, QShortcut
...
class MainWindow(QMainWindow):
    ...
    def get_playback_context_anchor_index(self, ordered_paths: list[str]) -> int:
    ...
    def get_playback_navigation_indices(self, direction: int) -> list[int]:
        ordered_paths = self.get_playback_context_paths()

        if not ordered_paths:
            return []

        current_index = self.get_playback_context_anchor_index(ordered_paths)
    ...
    def play_from_playback_context(self, direction: int) -> bool:
        ...
        for target_index in self.get_playback_navigation_indices(direction):
