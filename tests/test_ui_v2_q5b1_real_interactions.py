...
class Q5B1RealInteractionTests(unittest.TestCase):
    ...
    def test_quick_settings_auto_saves_and_survives_panel_dismissal(self) -> None:
        shell = self._immersive_shell("immersive_lyrics")
        self.assertIsInstance(shell, ImmersiveLyricsPage)
        ...
        self.assertEqual(saved["immersive_lyrics_font_scale"], preview_scale)
        self.assertEqual(saved["immersive_background_transparency"], 44)

        panel.close_button.click()
        self.app.processEvents()
        shell.show_settings_panel()
        self.app.processEvents()
        normalized_saved_scale = int(round(preview_scale / 5.0) * 5)
