...
Issues live in this repo's GitHub Issues and use the `gh` CLI. See `docs/agents/issue-tracker.md`.
...
This is a single-context repository; domain docs use root `CONTEXT.md` and `docs/adr/`. See `docs/agents/domain.md`.
...
`CHANGELOG.md` 是 HushPlayer 用户可见更新日志的唯一事实来源。

- 所有已确认、面向用户的新增、改进、性能优化和修复必须先写入 `## 未发布` 章节；未发布章节不得出现在在线更新清单中。
- 准备发布前，必须将已验收的内容整理为正式版本章节，并写明版本号、发布日期、Git 标签或发布源码提交（信息可得时）以及 `### 在线更新摘要`。
- `### 在线更新摘要` 必须是纯文本项目列表；发布期工具据此生成或校验更新清单中的 `release_notes` 和可选 `release_history`，运行时不得解析完整 `CHANGELOG.md`。
- 修改正式版本号、创建正式构建、创建标签、创建 Release、生成或更新在线更新清单、开始在线发布之前，必须运行更新日志校验。缺少 `CHANGELOG.md`、未发布章节、目标正式版本章节或在线更新摘要时，发布流程必须停止。
- 已撤销、未合并、未验收、未发布、纯内部重构或仅用于测试的改动不得伪装成用户功能写入正式版本章节或在线更新清单。
- 保留 `release_notes: string[]` 以兼容旧客户端；新增 `release_history` 时，客户端跨越多个版本更新必须按版本逐项展示日志，不能合并为一组或遗漏中间版本。
- 如果历史清单条目损坏，客户端应记录诊断信息并回退到目标版本的 `release_notes`，不得阻止更新；跨版本筛选必须使用四段 `numeric_version`，不得比较版本字符串。
