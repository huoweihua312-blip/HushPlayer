...
BACKUP_FILE = PROJECT_ROOT / "app" / "ui" / "main_window.py.bak_v0483"
...
NEW_IMMERSIVE_CLASS = r"""class ImmersiveLyricsWindow(QWidget):
    def __init__(self, main_window) -> None:
        ...
        self.alpha_label = QLabel("背景不透明度 82%")
        ...
        self.footer = QLabel("Esc 退出沉浸 · 当前：半透明背景 · 可拖动右上角背景不透明度")
    ...
    def apply_immersive_style(self) -> None:
        ...
        if self.transparent_mode:
            ...
            footer_text = f"Esc 退出沉浸 · 当前：半透明背景 · 背景不透明度 {self.background_alpha}%"
            button_text = "切换纯黑"
            slider_enabled = True
        else:
            background = "#050609"
        ...
        self.alpha_label.setText(f"背景不透明度 {self.background_alpha}%")
...
def replace_immersive_class(text: str) -> str:
    ...
    if not pattern.search(text):
        raise RuntimeError("没有找到 ImmersiveLyricsWindow 类。请先确认已经升级到 v0.4.8.3。")
...
def ensure_version(text: str) -> str:
    ...
    for old in old_versions:
        text = text.replace(old, "HushPlayer/0.4.8.4 (local music player prototype)")
...
def main() -> None:
    ...
    if "HushPlayer/0.4.8.4" in text:
        print("当前文件看起来已经升级到 v0.4.8.4 了，不需要重复升级。")
        return
    ...
    print("升级完成：v0.4.8.4 滑块已改为只调节背景透明度，不再影响歌词深浅。")
