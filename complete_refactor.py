#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Complete refactor of settings_dialog.py with new navigation layout"""
...
# Find the insertion point (where appearance_card starts)
insert_point = None
for i, line in enumerate(lines):
    if 'appearance_card = QFrame()' in line:
        insert_point = i
        break

# Find where button_row ends
button_row_end = None
for i in range(insert_point, len(lines)):
    if 'layout.addLayout(button_row)' in lines[i]:
        button_row_end = i
        break

print(f"Removing lines {insert_point + 1} to {button_row_end + 1}")

# Split file into: header (before old UI) + new UI code + footer (after old UI)
header = lines[:insert_point]
footer = lines[button_row_end + 1:]

# New init continuation and page methods
new_code = '''
        # Connect navigation
...
# Combine
result = '\n'.join(header) + new_code + '\n' + '\n'.join(footer)

# Write
Path('app/ui/settings_dialog.py').write_text(result, encoding='utf-8')
print(f"Refactored! New file has {len(result.splitlines())} lines")
